...
import { ALLOW_HEADER, FORBIDDEN_METHODS, refuseUnparsedForbiddenMethods } from './runtime/handler/http-helpers.js';
...
function createNodeApp() {
	...
	refuseUnparsedForbiddenMethods(httpServer);
	/** @type {any} */
	let behavior = {};
	let wsPathN = '/ws';
...
export async function createTestServer(options = {}) {
	...
	app.ws(wsPath, {
		...
		open(ws) {
			...
			// Same reason as the production handler: claim the slots as own
			// properties before anything writes one, so an accessor on the
			// prototype chain cannot swallow the write.
	...
	if (adminPath !== false && typeof handler.admin === 'function') {
		app.any(adminPath + '/*', (res, req) => {
			...
			if (FORBIDDEN_METHODS.has(method)) {
				// The same 405 the production helper writes, in the response
				// shape this server mirrors.
				res.cork(() => {
