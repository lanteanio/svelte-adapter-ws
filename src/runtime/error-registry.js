...
export const ADAPTER_ERROR_IDS = Object.freeze({
	...
	TLS_WATCH_LOST: 'ADAPTER-ERR-TLS-WATCH-LOST',
	SUBSCRIPTION_SINK_DISPLACED: 'ADAPTER-ERR-SUBSCRIPTION-SINK-DISPLACED',
	...
	CLUSTER_CONFIG_PORT: 'ADAPTER-ERR-CLUSTER-CONFIG-PORT',
	...
	TLS_PRIMARY_WATCH_LOST: 'ADAPTER-ERR-TLS-PRIMARY-WATCH-LOST',
...
export const ADAPTER_ERROR_REGISTRY = Object.freeze([
	...
	Object.freeze({
		id: ADAPTER_ERROR_IDS.SUBSCRIPTION_SINK_DISPLACED,
		code: null,
		event: 'runtime.subscription-accounting.sink-displaced',
		component: 'runtime.subscription-accounting',
		severity: 'warn',
		emission: 'direct',
		problemPrefix: 'A second adapter runtime in this worker took over the subscription accounting sink. One subscription total is now frozen and the other is charged releases it never matched.',
		messagePrefix: direct('runtime.subscription-accounting', 'runtime.subscription-accounting.sink-displaced', 'warn', 'A second adapter runtime in this worker took over the subscription accounting sink. One subscription total is now frozen and the other is charged releases it never matched.'),
	...
	Object.freeze({
		id: ADAPTER_ERROR_IDS.TLS_WATCH_LOST,
		code: null,
		event: 'tls.watch-lost',
		component: 'runtime.tls',
		severity: 'error',
		emission: 'direct',
		problemPrefix: 'The certificate directory watch stopped after running; hot reload is disabled and no further renewal will be seen.',
		messagePrefix: direct('runtime.tls', 'tls.watch-lost', 'error', 'The certificate directory watch stopped after running; hot reload is disabled and no further renewal will be seen.'),
		...
		automaticRecovery: 'None. The watch is not re-armed, so this does not resolve without a restart.',
		nextAction: 'Find what happened to the certificate directory - a redeployed secret volume that replaced the directory rather than the files in it is the usual cause - and restart the process. Until then, treat certificate renewal as requiring a restart and alert on certificate expiry independently.',
		sources: Object.freeze(['src/runtime/handler/tls.js']),
		anchor: 'adapter-err-tls-watch-lost',
		help: 'docs/errors.md#adapter-err-tls-watch-lost'
	}),
	Object.freeze({
	...
	Object.freeze({
		id: ADAPTER_ERROR_IDS.CLUSTER_CONFIG_PORT,
		code: null,
		event: 'cluster.config.ephemeral-port',
		component: null,
		severity: 'fatal',
		emission: 'console',
		problemPrefix: null,
		messagePrefix: '[svelte-adapter-ws] PORT=0 cannot be combined with CLUSTER_WORKERS (each worker would bind its own kernel-assigned port, so the workers do not share one: ',
		...
		automaticRecovery: 'None. The primary exits with status 1 before spawning any worker, the way every other capacity misconfiguration in this block does.',
		nextAction: 'Set PORT to a real port when clustering. If an ephemeral port is what you want - a test harness, a sandbox - drop CLUSTER_WORKERS to run single-process, where one bind makes an ephemeral port meaningful again.',
		sources: Object.freeze(['src/runtime/index.js']),
		anchor: 'adapter-err-cluster-config-port',
		help: 'docs/errors.md#adapter-err-cluster-config-port',
	...
	Object.freeze({
		id: ADAPTER_ERROR_IDS.TLS_PRIMARY_WATCH_LOST,
		code: null,
		event: 'tls.primary.watch-lost',
		component: null,
		severity: 'error',
		emission: 'console',
		problemPrefix: null,
		messagePrefix: '[tls] primary cert watch stopped after running, cluster hot-reload disabled (server keeps running)',
		...
		automaticRecovery: 'None. The watch is not re-armed, so this does not resolve without a restart.',
		nextAction: 'Find what happened to the certificate directory and restart the primary. Until then, treat certificate renewal as requiring a restart and alert on certificate expiry independently. A watch that never started at all is reported separately as ADAPTER-ERR-TLS-PRIMARY-WATCH.',
		sources: Object.freeze(['src/runtime/index.js']),
		anchor: 'adapter-err-tls-primary-watch-lost',
		help: 'docs/errors.md#adapter-err-tls-primary-watch-lost'
	}),
	Object.freeze({
	...
	Object.freeze({
		...
		emission: 'console',
		problemPrefix: null,
		messagePrefix: '[svelte-adapter-ws] a sveltekit:shutdown listener threw',
	...
	Object.freeze({
		...
		emission: 'console',
		problemPrefix: null,
		messagePrefix: '[svelte-adapter-ws] in-flight requests did not finish within the shutdown budget (',
	...
	Object.freeze({
		...
		nextAction: 'Fix the hook, then check whatever it was flushing for state left behind. The hook is awaited with { platform, signal, deadline } during the drain and shares the shutdown cleanup budget, so long-running flushes must finish inside it.',
		sources: Object.freeze(['src/runtime/handler/realtime.js']),
	...
	Object.freeze({
		...
		sources: Object.freeze(['src/runtime/handler/realtime.js', 'src/testing.js']),
	...
	Object.freeze({
		...
		nextAction: 'This is an adapter-internal failure - report it with the error printed beside it. The console write is the candidate to look at first: the export push after it contains its own failures (a non-serializable snapshot and a slow client are both handled inside it).',
	...
	Object.freeze({
		...
		nextAction: 'The line names the structure. Find the path that stopped shedding for it - a close, unsubscribe or eviction that no longer runs - rather than raising a limit.',
		sources: Object.freeze(['src/runtime/handler/realtime.js']),
