...
**What to do.** Check address availability, port conflicts, and bind permissions, then restart the process. In reuseport mode a repeating restart line for the same worker slot beside this one is the same conflict burning restart budget, not a second fault.
...
**What to do.** Install the exact supported archive and a binary for the active OS, CPU, Node ABI, and documented Linux libc floor.

Further reading: https://svti.me/native-load
...
**What to do.** Identify the topic from the attributes and decide whether the rate is intended. Because the line is suppressed entirely while an onPublishRate listener is registered and otherwise throttled per topic, its absence is not evidence the condition ended - read the pressure metrics for that. Left alone, a runaway publisher is what later produces slow-consumer disconnects on unrelated topics.
...
**What to do.** From a worker, treat this as an outage countdown: fix the certificate files now, and restart the instance if the reload cannot be repaired before the printed expiry. From the primary, verify the served certificate first (see ADAPTER-ERR-TLS-PRIMARY-RELOAD-READ) before treating the countdown as real.
...
**What to do.** The line names the structure. Find the path that stopped shedding for it - a close, unsubscribe or eviction that no longer runs - rather than raising a limit; framework_resource_growth_suspected_total carries the structure as its `resource` label and shows whether the trend continued.
